...
function mark_four_positions!(robot)
    for sides in ((West, Sud), (Nord, West), (Ost, Nord), (Sud, Ost))
        ...
        #Робот дошёл до первого угла
        get_point_and_return!(robot, path, inverse(sides[2]))
        #Робот дошёл до нужной точки, поставил маркер и вернулся в угол
        move_to_back!(robot, path)
        #Робот на исходной позиции
    end
    #Робот повторил последовательность действий 4 раза и закрасил все маркеры в нужных позициях
end
